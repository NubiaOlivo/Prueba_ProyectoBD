    ...
    public partial class Cuenta : Form
    {
        ...
        private void MostrarFormularioCambioDireccion()
        {
            ...
            "Paraguay", "Perú", "República Dominicana", "Surinam", "Uruguay", "Venezuela" });
                ...
                string idDireccion=direccionSeleccionada.ID_Direccion.ToString();

                // Guardar los cambios realizados
                btnGuardarDireccion.Click += (s, e) => GuardarCambioDireccion(txtDireccion.Text, txtCodigoPostal.Text, cmbPais.Text, txtTelefono.Text, idDireccion);
        ...
        private void GuardarCambioDireccion(string direccion, string codigoPostal, string pais, string telefono, string idDireccion)
        {
            string query = "UPDATE DireccionesEnvio " +
                   "SET Dirección = @Direccion, CodigoPostal = @CodigoPostal, País = @Pais, Telefono = @Telefono " +
                   "WHERE ID_Usuario = @IDUsuario AND ID_DireccionEnvio = @IDDireccion";
